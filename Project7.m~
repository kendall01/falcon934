...
for i = 1:length(phi)
    ...
    S1 = entropy_mole(state_ref); %define original enthalpy to reference bc nozzle is isentropic
    ...
    while h_0 < RHS
        ...
        state_new = setState_SP(state_ref, SP); %should keep all same, but set gas to a new pressure
        state_new = equilibrate(state_new, 'SP'); %should equilibrate gas to a all new thermo props given constant S and same new P
